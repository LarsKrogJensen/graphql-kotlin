...
class UnionTest extends Specification {
    ...
    def "can introspect on union and intersection types"() {
        ...
        then:
        executionResult.toCompletableFuture().get().data() == expectedResult
    ...
    def "executes union types with inline fragments"() {
        ...
        when:
        def executionResult = GraphQL.newGraphQL(GarfieldSchema.GarfieldSchema).build().execute(query, null, GarfieldSchema.john, [:])
